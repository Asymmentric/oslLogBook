...
//player1 = p1 
 ...
 // p1HealthDiv.innerText=p1.health
  ...
  p1.health = Math.min(p1.health, 100);
      p2.health = Math.min(p2.health, 100);
...
return `${player.name} attacks ${enemy.name} for ${damageAmount} damage `
  ...
  heal (player) {
    ...
    let amountToMaxHealth = 100 - player.health;
    // Calculate the actual amount to heal (minimum of hpAmount and amountToMaxHealth)
    let actualHealAmount = Math.min(hpAmount, amountToMaxHealth);
    // Add the actualHealAmount to the player's health
    player.health += actualHealAmount;
// how do we get  the health 
    //  Update the game and DOM with updateGame()
    updateGame(p1,p2,gameState)
...
let player1=new Player (`xx`,100,5)

let player2=new Player (`yy`,100,5)
